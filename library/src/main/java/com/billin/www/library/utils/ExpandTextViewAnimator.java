 ...
 */
package com.billin.www.library.utils;
...
public class ExpandTextViewAnimator {

    private static final String TAG = "ExpandTextViewAnimator";
    ...
    @Nullable
    public static ValueAnimator setMaxLinesWithAnimation(@NonNull final TextView textView,
                                                         final int maxLine) {

        // 测量 TextView 设置为 maxLine 的高度。原理是（通过创建 StaticLayout 来进行行高的获取，
        // 创建 StaticLayout 的代码位于 #createStaticLayoutForMeasuringCompat, 在这里面创建 StaticLayout 需要通过反射获取 TextView 的 Alignment 参数
        // 获取行高的核心代码是调用 StaticLayout#getLineTop 方法。）
        ...
        // 一些细节 - StaticLayout 获取的高度仅仅是文字显示的高度，还需要考虑 TextView 中 Drawable 的高度
        final int minLines = textView.getMinLines();
        final int targetHeight = textHeight
                + textView.getCompoundPaddingBottom()
                + textView.getCompoundPaddingTop();

        // 创建一个更改 TextView Height 的动画，并且设置动画结束的时候 settingMaxLine
    ...
    @Nullable
    public static ValueAnimator animatorToHeight(@NonNull final TextView textView,
                                                 int h,
    ...
    public static int measureTextHeight(@NonNull TextView textView, @NonNull CharSequence text) {
        return measureTextHeight(textView, text, textView.getMaxLines());
    }

    public static int measureTextHeight(@NonNull TextView textView,
                                        @NonNull CharSequence text, int maxLines) {
    ...
    public static int measureTextHeight(@NonNull TextView textView,
        ...
        final boolean horizontallyScrolling = invokeAndReturnWithDefault(textView,
        ...
        final StaticLayout layout = createStaticLayoutForMeasuringCompat(text, alignment,
                Math.round(TEMP_RECT_F.right), Integer.MAX_VALUE, textPaint, textView);
        return layout.getLineTop(Math.min(layout.getLineCount(), maxLines));
    }

    private static StaticLayout createStaticLayoutForMeasuringCompat(CharSequence text,
    ...
    private static StaticLayout createStaticLayoutForMeasuring(CharSequence text,
                                                               Layout.Alignment alignment,
        ...
        final TextDirectionHeuristic textDirectionHeuristic = invokeAndReturnWithDefault(textView,
                "getTextDirectionHeuristic", TextDirectionHeuristics.FIRSTSTRONG_LTR);

        final StaticLayout.Builder layoutBuilder = StaticLayout.Builder.obtain(text,
                0, text.length(), textPaint, availableWidth);
    ...
    @RequiresApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH)
    private static StaticLayout createStaticLayoutForMeasuringPre23(CharSequence text,
                                                                    Layout.Alignment alignment,
                                                                    int availableWidth,
                                                                    TextPaint textPaint,
                                                                    TextView textView) {
